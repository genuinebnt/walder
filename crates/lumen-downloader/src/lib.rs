...
impl DownloadManager {
    ...
    pub async fn cancel(&self, task_id: Uuid) -> lumen_core::Result<()> {
        ...
        if let Some(task) = tasks.get_mut(&task_id) {
            ...
            Err(LumenError::NotFound(format!(
    ...
    async fn download_file(
        ...
        let resp = request
            .send()
            .await
            .map_err(|e| LumenError::Download(e.to_string()))?;

        if !resp.status().is_success() {
            return Err(LumenError::Download(format!("HTTP {}", resp.status())));
        }
        ...
        while let Some(chunk) = stream.next().await {
            let chunk = chunk.map_err(|e| LumenError::Download(e.to_string()))?;
