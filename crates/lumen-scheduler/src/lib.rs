...
impl Scheduler {
    ...
    pub async fn reload_config(&self) -> lumen_core::Result<()> {
        self.tx
            .send(SchedulerCommand::UpdateConfig)
            .await
            .map_err(|_| LumenError::Scheduler("Failed to send reload command".into()))
    }
        ...
        let paths = match &config.source {
            ...
            SchedulerSource::BookmarkFolder(id) => {
                ...
                if bookmarks.is_empty() {
                    return Err(LumenError::Scheduler(
    ...
    async fn scan_directory(dir: &Path) -> lumen_core::Result<Vec<PathBuf>> {
        if !dir.exists() || !dir.is_dir() {
            return Err(LumenError::Scheduler(format!(
                "Directory not found: {}",
                dir.display()
            )));
        }
        ...
        while let Some(entry) = entries.next_entry().await.map_err(LumenError::Io)? {
            let path = entry.path();
            if path.is_file() {
