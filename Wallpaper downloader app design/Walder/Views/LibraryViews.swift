...
struct DownloadsView: View {
    ...
    private func row(_ task: DownloadTask) -> some View {
        HStack(spacing: Tokens.s3) {
            AsyncImage(url: task.wallpaper.thumb) { $0.resizable().scaledToFill() }
            ...
            VStack(alignment: .leading, spacing: 6) {
                HStack(spacing: Tokens.s2) {
                    ...
                    Text(task.wallpaper.sizeMB).font(.system(size: 11)).foregroundStyle(.secondary)
                }
            ...
            Button("Set") { store.setWallpaper(task.wallpaper) }
