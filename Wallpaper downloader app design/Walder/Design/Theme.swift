...
// Design tokens. Values mirror the palette already used by the Rust app
// (src/theme.rs) so the two front ends stay visually identical.
enum Tokens {
    static let accent = Color(nsColor: .controlAccentColor)          // follows System Settings
    static let wallhavenBlue = Color(red: 0.039, green: 0.518, blue: 1.0)   // #0A84FF
    ...
    // Motion. One curve family, three speeds — the whole app animates with these.
